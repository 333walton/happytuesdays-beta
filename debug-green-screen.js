...
const checkComponentLoading = () => {
  ...
  const errorBoundaries = document.querySelectorAll('[data-react-error], .react-error');
  console.log(`Error boundaries found: ${errorBoundaries.length}`);
  
  // Check for Hydra98 specific elements
  const hydraElements = document.querySelectorAll('[class*="hydra"], [class*="w98"], [class*="desktop"]');
  console.log(`Hydra98/W98 elements found: ${hydraElements.length}`);
};
...
const tryFix = () => {
  ...
  const body = document.body;
  if (body.style.backgroundColor === 'green' || window.getComputedStyle(body).backgroundColor === 'rgb(0, 128, 0)') {
  ...
  const crtElements = document.querySelectorAll('.crt, .screen-effect, .monitor-effect');
