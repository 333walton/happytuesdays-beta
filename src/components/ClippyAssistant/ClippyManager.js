...
// Import chatResponses when needed
...
class ClippyManager {
  ...
  initialize(ClippyService, isMobile = false) {
    ...
    const extraYPercent = isMobile ? 40 / 600 : 40 / 800; // ~6.7% for mobile, 5% for desktop

    ClippyService.setInitialPosition({
      position: `${isMobile ? 85 : 90}% ${
        (isMobile ? 85 : 50) + extraYPercent * 100
      }%`,
    });
  ...
  removeDuplicateClippys() {
    ...
    if (clippyElements.length <= 1) return;
  ...
  // Find Clippy element in the document
  getClippyElement() {
    const mainContainer =
      document.querySelector(".desktop.screen") ||
      document.querySelector(".w98");
    let clippyEl = mainContainer
      ? mainContainer.querySelector(".clippy")
      : null;

    // Fall back to any Clippy if not found in container
    if (!clippyEl) {
  ...
  enforceFixedPositionOnDesktop() {
    ...
    // Find Clippy element
    const clippyEl = this.getClippyElement();

    // If no Clippy element found, retry a few times
    if (!clippyEl) {
    ...
    this.createClickableOverlay(rect);
    ...
    // Setup the position tracking animation frame
    this.setupPositionTracking(clippyEl);
  }

  // Create overlay for handling clicks
  createClickableOverlay(rect) {
    if (this.clippyOverlay) return;
      ...
      if (window.clippy) {
        ...
        setTimeout(() => {
          ...
          if (window._clippyDoubleClickCount === 0) {
            // Show custom balloon with speech and options after animation
            setTimeout(() => {
  ...
  // Create context menu at specified position
  ...
  // Setup position tracking animation frame
  setupPositionTracking(clippyEl) {
  ...
  processUserInput(text, inputField) {
    ...
    let response =
      "I'm not sure how to help with that. Can you try asking something about Hydra 98?";
  ...
  positionCustomBalloon() {
    ...
    const clippyElement = this.getClippyElement();
  ...
  showChatBalloon(initialMessage) {
    ...
    return this.showCustomBalloon(initialMessage, false, options);
  }
  ...
  cleanup() {
    ...
    // Remove all Clippy instances and overlays
    document.querySelectorAll(".clippy").forEach((clippy) => {
      if (clippy.parentNode) clippy.parentNode.removeChild(clippy);
    });

    document
      .querySelectorAll("#clippy-clickable-overlay")
      .forEach((overlay) => {
